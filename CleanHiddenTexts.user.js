...
// @description   clean hidden texts. Hotkey:CRTL+Q to remove hidden contents only, and CRTL+SHIFT+Q to enable right-click additionally. Created By Inorth. blueicejin@163.com.http://notebk.spaces.live.com/
// @include       *
// @version       0.5
...
// @updateURL     https://github.com/zephyrer/userscripts/raw/master/CleanHiddenTexts.meta.js
// @run-at        document-end
...
function keys(evt)  
  {  //var evt=evt?evt:(window.event?window.event:null);
      if(evt.ctrlKey && evt.keyCode=="81")  // 81: Q
      {  //var CleanHideObj=new CleanHideClass();
      	  CleanHideObj.on(0,1);
      	  
      	  //add enable right-click and copy feature
      	  if (evt.shiftKey) antiCopy();
      	  //end
...
var level = /\w{3}\.\w{2}$/.test(location.hostname) ? -3 : -2;
var domain = location.hostname.split('.').slice(level).join('.');
if (sites[domain] && sites[domain].rule.test(location.href)) {
		CleanHideObj.on(0,1);
}
